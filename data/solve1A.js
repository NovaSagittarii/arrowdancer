...
class Grid {
    ...
    verifyGridState(gridState, conditionSet, previousState){
        ...
        if(previousState) previousState.forEach((g,i) => loc[g] = i !== activeBlock ? true : loc[g]); // afterimage algorithm
        for(const condition of conditionSet){
            if(!condition.validate(pos)) return false;
            if(!condition.anchored)
                if(!loc[pos + condition.delta]) return false;
        }
        return true;
    }
    ...
    findCycle(initialGridStates, steps=1, fastTech=true, initialDanceState=this.initialState){
        ...
        const init = initialGridStates.map(g => [Grid.encodeGridState(g, initialDanceState), Grid.encodeGridState(g, ids)]);
        let solutions = [];
    ...
    // Grid.res[danceState] = Grid.res[danceState] ? Grid.res[danceState]+1 : 1;
                    ...
                    for(const ext of this.generateExtensions(g, steps, fastTech)){
                        const next = Grid.encodeGridState(ext, danceState);
    ...
    for(let i = 0; i < danceIndex.length; i ++){
        for(let j = 0; j < danceIndex.length; j ++){
            for(let k = 0; k < danceIndex.length; k ++){
                ...
                C[0].moveable = false;
                var grid = new Grid([A, B, C, Block.frog()])
                ...
                C2[0].moveable = false;
                var reversed = new Grid([A2, C2, B2, Block.frog()])
                ...
                var out = o+","+atk
                +","+(grid.solve(1, false)[0]?.length-2||99) +","+ (reversed.solve(1, false)[0]?.length-2||99)
                ...
                console.log(ct1, ++ctx);
                if(!out.endsWith("99,99")){
                    fs.appendFileSync('output-frog2A+C', out+'\n');
                }
                //break;
            }//break;
        }//break;
